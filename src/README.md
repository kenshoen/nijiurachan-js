...
(※entrypointsはアプリ側にあるのでこのリポジトリには無いです)
...
(※entrypointsはアプリ側なのでこのリポジトリには無いです)
...
- PHP版本体の作業をする分にはDockerに入っているのでインストールしなくても大丈夫です
- 主要なコマンドは下です
    1. `bun fix` でフォーマット
    2. `bun tsc` で型チェック
    3. `bun vitest` でテスト
    4. `bun clean` でビルド物を掃除
...
- elementsにとりあえず置いてください
    - お手すきなら上のモジュール分担通りにioやpureに分割したりしてテストしてください
...
- やむを得ずJSとTSでやり取りが必要な場合はカスタムイベントで連携してください
    ...
    - カスタムイベントの一覧は src/components/types.ts にあります

### 新規JSは追加していいですか？

- 完全な新機能の場合: TSでやりましょう
- JSに手を加えたい場合: TSにまず// @ts-nocheck つきで持ってきて、そこから手を加えましょう
    - `connectedCallback()`から初期化処理を呼べば同じ動きになるはずです
...
- ううｎの好みです
- 手元で作業するときは`biome.json`を好きに調整いただいて大丈夫です
- 生JSではちゃんとセミコロンを書きますので許してください
